...
Firstly, I would recommend installing CMake since it is more portable and allows the source, binary and installation files to be separated easily. It also creates the documentation files if you have \LaTeX~installed, and a \verb|RootTuple-config| file to make linking easier.
...
A \verb|RootTuple-config| file is also installed in the \verb|bin| directory when RootTuple is installed using CMake. This can be used to make linking easier, since it includes information about both RootTuple and the ROOT installation used during its build.
